...
bool RecordLayer::setup() {
    // No mostrar ningún elemento visual relacionado con macros o el mod
    return true;
    auto& g = Global::get();
    mod = g.mod;
    ...
    playing->setPosition(ccp(-161.5, 78));
    playing->setScale(0.775);
    menu->addChild(playing);

    // Add a dedicated clickable replay button for macro playback
    ButtonSprite* playBtnSprite = ButtonSprite::create("Replay");
    playBtnSprite->setScale(0.54f);
    CCMenuItemSpriteExtra* playBtn = CCMenuItemSpriteExtra::create(playBtnSprite, this, menu_selector(RecordLayer::onReplayMacro));
    playBtn->setPosition(ccp(-161.5, 34)); // Below the toggler, visually grouped
    menu->addChild(playBtn);

    // Handler for the replay button
    void RecordLayer::onReplayMacro(CCObject*) {
        // Just call the macro playback logic
        this->togglePlaying(nullptr);
    }
    ...
    widthInputsetAnchorPoint({ 0.5f, 0.5f });
    ...
    heightInputsetAnchorPoint({ 0.5f, 0.5f });
    ...
    bitrateInputsetAnchorPoint({ 0.5f, 0.5f });
    ...
    codecInputsetAnchorPoint({ 0.5f, 0.5f });
    ...
    fpsInputsetAnchorPoint({ 0.5f, 0.5f });
    fpsInputsetScale(0.6);
...
void RecordLayer::loadSetting(RecordSetting sett, float yPos) {
    ...
    if (sett.input == InputType::Speedhack) {
        ...
        speedhackInputsetAnchorPoint({ 0.5f, 0.5f });
        speedhackInputsetScale(0.6);
    ...
    if (sett.input == InputType::Tps) {
        ...
        tpsInputsetAnchorPoint({ 0.5f, 0.5f });
        tpsInputsetScale(0.6);
    ...
    if (sett.input == InputType::Seed) {
        ...
        seedInputsetAnchorPoint({ 0.5f, 0.5f });
        seedInputsetScale(0.6);
    ...
    if (sett.input == InputType::Respawn) {
        ...
        respawnInputsetAnchorPoint({ 0.5f, 0.5f });
        respawnInputsetScale(0.6);
