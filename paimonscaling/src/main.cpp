...
class $modify(PauseLayer) {
  ...
  void onPracticeMode(CCObject * sender) {
    ...
    auto& g = Global::get();
    // Si no está grabando, iniciar grabación de macro
    if (g.state != state::recording) {
      Macro::toggleRecording();
    }
    // Reiniciar el nivel si ya estaba grabando o reproduciendo
  ...
  void onNormalMode(CCObject * sender) {
    ...
    // Si estaba grabando macro, detener grabación
    if (g.state == state::recording) {
      Macro::resetState(false);
    }
